...
def generate_response(prompt):
    ...
    return output
##build titleyear2info dict
with open('RSO/eval/redial_factinfo.json', 'r', encoding='utf-8') as file:
    movie_data = json.load(file)


titileyear2info = {}
for k, movie in movie_data.items()
    titileyear2info[movie["name"]] = movie


dialog_id2user_id_and_genre_dict = {}
with open('RSO/eval/redial_dialog_ids2user_id_and_genre_dict.jsonl', 'r', encoding='utf-8') as f:
    for line in f:
        line = line.strip()
        if not line:
            continue
        data = json.loads(line)
        dialog_id2user_id_and_genre_dict[data["dialog_id"]] = {
            "user_id":data["user_id"],
            "genre_dict":data["genre_dict"]
        }

def get_watching_intension(dialog_id,refer,type_):
    user_id = dialog_id2user_id_and_genre_dict[dialog_id]["user_id"]
    preferred_genres = [dialog_id2user_id_and_genre_dict[dialog_id]['genre_dict'][0]]
    if type_ == "pre":
...
def get_movie_nameyear(data):
    ...
    if len(all_nameyear) == 0:
        print("Warning: all_nameyear_emb is empty")
        return None
    all_nameyear_with_plot = []
    for nameyear in all_nameyear:
        all_nameyear_with_plot.append(f"{nameyear}:{titileyear2info[nameyear]['plot']}")
...
def get_persuasiveness_score_for_dialogue(dialog_id,data):
    print(data["full_state"][2]["Seeker_prompt"])
    movie_nameyear = get_movie_nameyear(data)
    if movie_nameyear is None:
        return {'user_id': user_id, 'movie_nameyear': movie_nameyear, 'pre': -1.0, 'after': -1.0, 'true': -1.0, 'persuasiveness': "None, because no movie nameyear is found"}
    movie_info = titileyear2info[movie_nameyear]
    pre_score = get_watching_intension(dialog_id, movie_nameyear,type_="pre")
    after_score = get_watching_intension(dialog_id, get_conversation_history(data),type_="after")
    true_score = get_watching_intension(dialog_id, movie_info,type_="true")
    ...
    return {'dialog_id': dialog_id, 'user_id': dialog_id2user_id_and_genre_dict[dialog_id]["user_id"], 'movie_nameyear': movie_nameyear, 'pre': pre_score, 'after': after_score, 'true': true_score, 'persuasiveness': persuasiveness}
...
def main():
    ...
    user_ids = []
    dialog_ids = []
    with open("RSO/eval/redial_dialog_ids2user_id_and_genre_dict.jsonl") as f:
        for line in f:
            test_ds.append(json.loads(line))
    for test_data in test_ds:
        user_ids.append(test_data["user_id"])
        dialog_ids.append(test_data["dialog_id"])
    with open(score_dir + f"persuasiveness_score.txt", 'w', encoding='utf-8') as f:
        for i in tqdm(range(len(datas)), desc="Computing scores"):
            ...
            scores = get_persuasiveness_score_for_dialogue(dialog_id, data)
            f.write(f"{scores}\n")
            f.flush()
