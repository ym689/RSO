...
import torch
import pickle
from prompt import ESConvAct, CIMAAct, CBAct, InspiredAct
import json

logger = logging.getLogger(__name__)

role_map = {'inspired':{'RECOMMENDER':'RECOMMENDER','SEEKER':'SEEKER'}}
act_map = {'inspired':InspiredAct}
...
def convert_to_features(args, tokenizer, mode):
    ...
    act = sorted(list(act_map[args.data_name].keys()))#change:define act_map in prompt.py
    ...
    with open(path, 'r', encoding='utf-8') as infile:
        ...
        if args.data_name == 'inspired':
            for line in infile:
                ...
                for turn in dial:
                    if turn['speaker'] == 'RECOMMENDER' and len(state) > 0:
                        dial_id = []
                        for s in state[::-1]:
